          ...
          <div v-if="syncProgress && (syncProgress.expectedChunks > 0 || syncProgress.txPagesFetched > 0)" class="space-y-2">
            ...
            <div v-if="syncProgress.expectedChunks > 0">
              <div class="flex justify-between text-xs mb-1">
                ...
                <span class="text-white font-medium">{{ syncProgress.chunksFound?.toLocaleString() || 0 }} / {{ syncProgress.expectedChunks.toLocaleString() }}</span>
              </div>
            ...
            <div v-if="cartHeader">
              <div class="flex justify-between text-xs mb-1">
                ...
                <span class="text-white font-medium">{{ formatBytes(syncProgress.bytes || 0) }} / {{ formatBytes(cartHeader.totalSize) }}</span>
              </div>
              <div class="w-full bg-gray-700 rounded-full h-2 overflow-hidden relative">
                <div
                  class="bg-green-600 h-full rounded-full transition-all duration-300 absolute left-0 top-0"
                  :style="{ width: cartHeader && cartHeader.totalSize > 0 ? `${Math.min(100, Math.max(0, ((syncProgress.bytes || 0) / cartHeader.totalSize * 100)))}%` : '0%' }"
                ></div>
              </div>
            </div>
            ...
            <div v-if="syncProgress.rate > 0" class="text-center pt-1 flex items-center justify-center gap-3">
              ...
              <span v-if="estimatedTimeRemaining && (syncProgress.chunksFound || 0) < syncProgress.expectedChunks" class="text-xs text-gray-400">
...
const estimatedTimeRemaining = computed(() => {
  ...
  const remaining = props.syncProgress.expectedChunks - (props.syncProgress.chunksFound || 0)
...
const combinedProgress = computed(() => {
  ...
  if (progress.expectedChunks > 0) {
    const chunkProgress = Math.min(1, (progress.chunksFound || 0) / progress.expectedChunks)
    
    // Chunks done - now reconstruction/verification (90-100%)
    if ((progress.chunksFound || 0) >= progress.expectedChunks) {
